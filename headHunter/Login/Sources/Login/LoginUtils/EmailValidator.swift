...
extension String {
    ...
    ///     - allowTopLevelDomains: Allow top level domains (e.g. `postmaster@dk`)
    ///     - allowInternational: Allow international addresses (e.g. cyrillic or chinese)
    /// - Returns: Bool indicating whether the string is a valid email address or not
    func isValidEmail(allowTopLevelDomains: Bool = false, allowInternational: Bool = false) -> Bool {
        return EmailValidator.validate(email: self)
    }
}
